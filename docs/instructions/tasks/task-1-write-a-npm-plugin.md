...
Your first task is to implement an **NPM plugin for Depsight** that exposes a `depsight npm scan` command. Completing this task is a prerequisite for [Task 2](task-2-scan-dependencies.md), where you will use your plugin to run a full dependency scan on the `package.json` of the [target NPM project](../getting-started/getting-started.md#clone-the-target-npm-repository) and report its dependencies. 
...
![Todos Overview](../../images/todos_overview.png)
...
!!! warning "Skip the Dockerfile TODO"
    Implement all TODOs **except** the one inside the `Dockerfile`. That TODO is covered in [Task 3](task-3-containerize-your-application.md).
